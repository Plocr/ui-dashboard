...
var plugin = {
  ...
  apply(ctx) {
    ...
    function BalanceCard({ t }) {
      ...
      const kind = state.data !== null && state.data.kind === 'usage' ? 'usage' : 'money'
      const kids = [
        React.createElement('div', { className: 'udash-cardHeadRow', key: 'head' }, [
          cardHead('coin', kind === 'usage' ? t('usage.title') : t('balance.title')),
    ...
    ctx.slots.inject('details', () => ctx.slots.register(
      { name: 'details', priority: -100, locale: NS, inject: () => ({ closeDetails: () => ctx.layout.closeDetails() }) },
      DashboardPanel
    ))
